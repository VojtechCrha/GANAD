...
def pategan(x_train, parameters):
    ...
    k = int(parameters['k'])
    # epsilon
    epsilon = parameters['epsilon'] * 1 - parameters['ad_budget']
    # delta
    ...
    anomaly_detector = DP_Autoencoder(x_train.shape[1], int(x_train.shape[1] / 2), epsilon=parameters['epsilon'] * parameters['ad_budget'])
    ...
    while epsilon_hat < epsilon:
        ...
        for _ in range(k):
            ...
            G_mb = sess.run(G_sample, feed_dict={Z: Z_mb}) #Synth data

            temp_x = x_partition[i]
            idx = np.random.permutation(len(temp_x[:, 0]))
            X_mb = temp_x[idx[:partition_data_no], :] #Real data


            G_mb_indices = anomaly_detector.get_top_anomalies(G_mb, num_anomalies=num_anomalies)
            clean_G_mb = np.delete(G_mb, G_mb_indices, axis=0)

            X_mb_indices = anomaly_detector.get_top_anomalies(X_mb, num_anomalies= num_anomalies)
            clean_X_mb = np.delete(X_mb, X_mb_indices, axis=0)

            X_comb = np.concatenate((clean_X_mb, clean_G_mb), axis=0) #Combine both real and synth
            Y_comb = np.concatenate((np.ones([partition_data_no-num_anomalies, ]), #Real is labeled one, synth is labeled zero
                                     np.zeros([partition_data_no-num_anomalies, ])), axis=0)
