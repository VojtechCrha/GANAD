...
def make_generator_model(noise_dim):
    ...
    model.add(layers.Dense(179, use_bias=False, input_shape=(noise_dim,)))
...
def train_step(generator, discriminator, gan, gen_optimizer, disc_optimizer, x_batch, batch_size, noise_dim):
    def gen_loss_fn():
        return gen_loss
...
# def calculate_fid(real_data, generated_data, batch_size=128):
#     # Load pre-trained InceptionV3 model without classification head
#     inception_model = tf.keras.applications.InceptionV3(include_top=False, pooling='avg', input_shape=(299, 299, 3))
#
#     # Resize images to (299, 299) required by InceptionV3
#     real_resized = tf.image.resize(real_data, (299, 299), method=tf.image.ResizeMethod.BILINEAR).numpy()
#     generated_resized = tf.image.resize(generated_data, (299, 299), method=tf.image.ResizeMethod.BILINEAR).numpy()
#
#     # Normalize pixel values to [0, 1]
#     real_resized = real_resized.astype('float32') / 255.0
#     generated_resized = generated_resized.astype('float32') / 255.0
#
#     # Get InceptionV3 feature representations
#     real_features = inception_model.predict(real_resized, batch_size=batch_size, verbose=1)
#     generated_features = inception_model.predict(generated_resized, batch_size=batch_size, verbose=1)
#
...
def dp_gan_main(args):
    ...
    synth_data_indices = anomaly_detector.get_top_anomalies(synth_data, num_anomalies=10)
    clean_synth_data = np.delete(synth_data, synth_data_indices, axis=0)
    return clean_synth_data, train_data, test_data
