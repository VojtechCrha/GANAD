...
def final_testing():
    for e in [0.1, 1, 10]:
        ...
        anomaly_detector = DP_Autoencoder(data.shape[1], int(data.shape[1] / 2), epsilon=ad_epsilon)
        ...
        number_of_tests = 10
        folder_name = f"results/post_gen_eps={args.epsilon}_dataset={args.dataset}_iters={args.iterations}_k={args.k}_ADBudget={args.AD_budget}_delta={args.delta}_ns={args.n_s}_lambda={args.lamda}_noise_rate={args.noise_rate}_batchsize={args.batch_size}"
        ...
        for n in range(1, number_of_tests + 1):
            ...
            if os.system(
                f"venv_directory/python autoencoder_postgen.py "
                f"--n_epochs_pretrain {ae_epochs} --noise_multiplier {ae_noise_mult} --batch_size 16") != 0:
                print("AE training failed, skipping test")
                continue
            print("Finished with training AE")
            if os.system(
                f"venv_directory/python wganprivate_postgen.py "
                f"--classLabel 0.0 --n_epochs {main_epochs} --noise_multiplier {noise_mult} --batch_size 16") != 0:
                print("First WGAN training failed, skipping test")
                continue

            if os.system(
                f"venv_directory/python wganprivate_postgen.py "
                f"--classLabel 1.0 --n_epochs {main_epochs} --noise_multiplier {noise_mult} --batch_size 16") != 0:
                print("Second WGAN training failed, skipping test")
                continue

            if os.system(
                f"venv_directory/python evaluate_postgen.py "
                f"--classLabel 0.0 --n_epochs {main_epochs} --noise_multiplier {noise_mult} --batch_size 16") != 0:
            ...
            gen_samples_0 = np.load("synthetic_postgen/synthetic_0.npy", allow_pickle=False)
            gen_samples_1 = np.load("synthetic_postgen/synthetic_1.npy", allow_pickle=False)
            synth_data = np.concatenate((gen_samples_0, gen_samples_1), axis=0)

            original_train_indices = anomaly_detector.get_top_anomalies(synth_data, num_anomalies=10)
            clean_train_data = np.delete(synth_data, original_train_indices, axis=0)
