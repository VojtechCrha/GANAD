    ...
    for e in [0.1, 1, 10]:
        ...
        for n in range(1, number_of_tests + 1):
            ...
            if os.system(
                ...
                f"--classLabel 0.0 --n_epochs {main_epochs} --noise_multiplier {noise_mult} --AD_budget {ad_epsilon} --use_midgen True --batch_size 16") != 0:
                print("First WGAN training failed, skipping test")
                continue

            if os.system(
                f"venv_directory/python wganprivate_midgen.py "
                f"--classLabel 1.0 --n_epochs {main_epochs} --noise_multiplier {noise_mult} --AD_budget {ad_epsilon} --use_midgen True --batch_size 16") != 0:
