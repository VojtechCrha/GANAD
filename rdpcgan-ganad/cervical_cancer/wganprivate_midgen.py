        ...
        for i_batch, samples in enumerate(dataloader_train):
            ...
            if opt.dp_privacy:
                ...
                if opt.use_midgen:
                    ...
                    if type(real_samples) == torch.Tensor:
                        real_samples = real_samples.cpu().numpy()
                    else:
                        pass

                    original_train_indices = anomaly_detector.get_top_anomalies(real_samples, num_anomalies=1)
                    real_samples = np.delete(real_samples, original_train_indices, axis=0)
                ...
                for i in range(microbatch_size):
                    ...
                    if opt.use_midgen:
                        ...
                        original_train_indices = anomaly_detector.get_top_anomalies(temp2, num_anomalies=1)
                        fake_decoded = np.delete(temp2, original_train_indices, axis=0)
                        fake_decoded = torch.from_numpy(fake_decoded).cuda()
