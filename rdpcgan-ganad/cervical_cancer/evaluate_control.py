...
experimentName = 'cervical_cancer'

parser.add_argument("--DATASETDIR", type=str,
                    default=os.path.expanduser('~/data/cervical_cancer'),
                    help="Dataset file")
...
parser.add_argument("--classLabel", type=float, default=0.0, help="Define the class label to train the model to generate")
...
parser.add_argument("--expPATH", type=str, default='synthetic_control/',
                    help="Experiment path")
parser.add_argument("--modelPATH", type=str, default='checkpoints_control/',
                    help="Model path")
...
class Autoencoder(nn.Module):
    ...
    def __init__(self):
        ...
        self.encoder = nn.Sequential(
            ...
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv1d(in_channels=2 * n_channels_base, out_channels=8 * n_channels_base, kernel_size=5, stride=3,
        ...
        self.decoder = nn.Sequential(
            ...
            nn.BatchNorm1d(n_channels_base),
            nn.LeakyReLU(0.2, inplace=True),
            nn.ConvTranspose1d(in_channels=n_channels_base, out_channels=1, kernel_size=4, stride=2,
                               padding=0, dilation=1,
                               groups=1, bias=True, padding_mode='zeros'),
            nn.Sigmoid(),
        )
...
class Generator(nn.Module):
    def __init__(self):
        ...
        nn.ConvTranspose1d(opt.latent_dim, ngf, 4, 1, 0),
...
class Discriminator(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Sequential(
            ...
            nn.Conv1d(1, ndf, 8, 5, 1),
            nn.LeakyReLU(0.2, inplace=True)
        )
        # self.conv2 = nn.Sequential(
        #     # state size. (ndf) x 32 x 32
        #     nn.Conv1d(ndf, ndf, 8, 4, 1),
        #     nn.BatchNorm1d(ndf),
        #     nn.LeakyReLU(0.2, inplace=True),
        # )
...
if opt.generate:
    ...
    #     )
        ...
        # Load optimizers @todo:check if needed
    ...
    latest_checkpoint = find_highest_x(opt.modelPATH, classLabel)
    checkpoint = torch.load(os.path.join(opt.modelPATH, f"model_epoch_{latest_checkpoint}_{int(classLabel)}.pth"))
...
if opt.evaluate:
    ...
    for model_index in range(len(models)):
        ...
        results[model_index, 0], results[model_index, 2] = (
            supervised_model_training(real_train[:, :(data_dim - 1)],
                                      np.round(real_train[:, (data_dim - 1)]),
                                      real_test[:, :(data_dim - 1)],
                                      np.round(real_test[:, (data_dim - 1)]),
                                      model_name))

        # Using synthetic data

        results[model_index, 1], results[model_index, 3] = (
            supervised_model_training(train_f[:, :(data_dim - 1)],
                                      np.round(train_f[:, (data_dim - 1)]),
                                      real_test[:, :(data_dim - 1)],
                                      np.round(real_test[:, (data_dim - 1)]),
                                      model_name))

        # Using original data without anomalies
        original_train_indices = anomaly_detector.get_top_anomalies_dp(real_train, ad_budget, num_anomalies=10)
        clean_train_data = np.delete(real_train, original_train_indices, axis=0)
        results[model_index, 4], results[model_index, 6] = (
            supervised_model_training(clean_train_data[:, :(data_dim - 1)],
                                      np.round(clean_train_data[:, (data_dim - 1)]),
                                      test_data[:, :(data_dim - 1)],
                                      np.round(test_data[:, (data_dim - 1)]),
                                      model_name))

        # Using synthetic data without anomalies
        synth_train_indices = anomaly_detector.get_top_anomalies_dp(synth_train_data_lower_eps, ad_budget,
                                                                    num_anomalies=10)
        clean_synth_train_data = np.delete(train_f, synth_train_indices, axis=0)
        results[model_index, 5], results[model_index, 7] = (
            supervised_model_training(clean_synth_train_data[:, :(data_dim - 1)],
                                      np.round(clean_synth_train_data[:, (data_dim - 1)]),
                                      real_test[:, :(data_dim - 1)],
                                      np.round(real_test[:, (data_dim - 1)]),
                                      model_name))
    ...
    results = pd.DataFrame(np.round(results, 4),
                           columns=['AUC-Original', 'AUC-Synthetic',
                                    ...
                                    'AUC - Original Clean', 'AUC - Synthetic Clean',
                                    'APR-Original Clean', 'APR-Synthetic Clean'])
    print(results)
    print('Averages:')
    ...
    # cls = GradientBoostingClassifier(n_estimators=n_estimator)
    #
    # # todo: Implement more than basic outlier detection
    # # print(X_train_f)
    # pytorch_tensor = torch.tensor(X_train_f, dtype=torch.float).cuda()
    #
    # print(pytorch_tensor.shape)
    # reconstructed = autoencoder_anomaly_Model(pytorch_tensor)[:, :178]
    # print(reconstructed.shape)
    # with torch.no_grad():
    #     reconstruction_errors = torch.square(reconstructed - pytorch_tensor)
    #
    # reconstruction_errors = reconstruction_errors.cpu().detach().numpy()
    #
    # # 0.05 is arbitrary, can be set to something else too
    # num_outliers = int(0.05 * len(reconstruction_errors))
    # outlier_indices = reconstruction_errors.argsort()[-num_outliers:]
    #
    # X_train_f_clean = np.delete(X_train_f, outlier_indices, axis=0)
    # y_train_f_clean = np.delete(y_train_f, outlier_indices, axis=0)
    #
    # # cls.fit(X_train_f_clean, y_train_f_clean)
