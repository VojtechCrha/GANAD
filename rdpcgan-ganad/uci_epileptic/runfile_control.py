...
def final_testing():
    for e in [0.1, 1, 10]:
        ...
        number_of_tests = 10
        folder_name = f"results/control_eps={args.epsilon}_dataset={args.dataset}_iters={args.iterations}_k={args.k}_ADBudget={args.AD_budget}_delta={args.delta}_ns={args.n_s}_lambda={args.lamda}_noise_rate={args.noise_rate}_batchsize={args.batch_size}"
        ...
        for n in range(1, number_of_tests + 1):
            ...
            os.system(
                f"venv_directory/python autoencoder_control.py "
                f"--n_epochs_pretrain {ae_epochs} --noise_multiplier {ae_noise_mult}")
            print("Finished with training AE")
            os.system(
                f"venv_directory/python wganprivate_control.py "
                f"--classLabel 0.0 --n_epochs {main_epochs} --noise_multiplier {noise_mult}")

            os.system(
                f"venv_directory/python wganprivate_control.py "
                f"--classLabel 1.0 --n_epochs {main_epochs} --noise_multiplier {noise_mult}")

            os.system(
                f"venv_directory/python evaluate_control.py "
                f"--classLabel 0.0 --n_epochs {main_epochs} --noise_multiplier {noise_mult}")
            ...
            gen_samples_0 = np.load("synthetic_control/synthetic_0.npy", allow_pickle=False)
            gen_samples_1 = np.load("synthetic_control/synthetic_1.npy", allow_pickle=False)
